...
public:
    ...
    inline void Draw2DElements(Camera3D& camera, int fromY, int toY)
    {
    ...
    inline RLModel ModelFromID(const ModelID id) const
    {
        if (id == NO_MODEL || id >= _modelList.size()) return RLModel{};
        return _modelList[id]->GetModel();
    }

    inline RLTexture TexFromID(const TexID id) const
    {
