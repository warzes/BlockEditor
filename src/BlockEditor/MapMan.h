...
public:
	...
	public:
		...
		void Do(MapMan& map) const final;
		void Undo(MapMan& map) const final;
	...
	const TileGrid& Tiles() const { return _tileGrid; }
	const EntGrid& Ents() const { return _entGrid; }

	void DrawMap(Camera3D& camera, int fromY, int toY);

	void Draw2DElements(Camera3D& camera, int fromY, int toY)
	{
	...
	bool ExportGLTFScene(std::filesystem::path filePath, bool separateGeometry);
	...
	std::filesystem::path PathFromTexID(const TexID id) const
	{
		if (id == NO_TEX || id >= _textureList.size()) return std::filesystem::path();
		return _textureList[id]->GetPath();
	}

	std::filesystem::path PathFromModelID(const ModelID id) const
	{
		if (id == NO_MODEL || id >= _modelList.size()) return std::filesystem::path();
		return _modelList[id]->GetPath();
	}

	RLModel ModelFromID(const ModelID id) const
	{
		if (id == NO_MODEL || id >= _modelList.size()) return RLModel{};
		return _modelList[id]->GetModel();
	}

	RLTexture TexFromID(const TexID id) const
	{
		if (id == NO_TEX || id >= _textureList.size()) return RLTexture{};
		return _textureList[id]->GetTexture();
	}

	const std::vector<std::shared_ptr<Assets::ModelHandle>> GetModelList() const { return _modelList; }
	const std::vector<std::filesystem::path> GetModelPathList() const;
	int GetNumModels() const { return _modelList.size(); }
	const std::vector<std::shared_ptr<Assets::TexHandle>> GetTextureList() const { return _textureList; }
	const std::vector<std::filesystem::path> GetTexturePathList() const;
	int GetNumTextures() const { return _textureList.size(); }

	Vector3 GetDefaultCameraPosition() const { return _defaultCameraPosition; }
	void SetDefaultCameraPosition(Vector3 pos) { _defaultCameraPosition = pos; }
	Vector3 GetDefaultCameraAngles() const { return _defaultCameraAngles; }
	void SetDefaultCameraAngles(Vector3 angles) { _defaultCameraAngles = angles; }
