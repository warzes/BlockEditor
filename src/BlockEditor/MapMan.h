...
public:
	...
	inline TexID GetOrAddTexID(const std::filesystem::path texturePath)
	{
	...
	inline ModelID GetOrAddModelID(const std::filesystem::path modelPath)
	{
		//Look for existing ID
	...
	inline std::filesystem::path PathFromTexID(const TexID id) const
	{
		if (id == NO_TEX || id >= _textureList.size()) return std::filesystem::path();
		return _textureList[id]->GetPath();
	}

	inline std::filesystem::path PathFromModelID(const ModelID id) const
	{
		if (id == NO_MODEL || id >= _modelList.size()) return std::filesystem::path();
		return _modelList[id]->GetPath();
	}

	inline StaticModelRef ModelFromID(const ModelID id) const
	{
		if (id == NO_MODEL || id >= _modelList.size()) return {};
		return _modelList[id]->GetModel();
	}

	inline Texture2DRef TexFromID(const TexID id) const
	{
		if (id == NO_TEX || id >= _textureList.size()) return {};
	...
	inline glm::vec3 GetDefaultCameraPosition() const { return _defaultCameraPosition; }
	inline void SetDefaultCameraPosition(glm::vec3 pos) { _defaultCameraPosition = pos; }
	inline glm::vec3 GetDefaultCameraAngles() const { return _defaultCameraAngles; }
	inline void SetDefaultCameraAngles(glm::vec3 angles) { _defaultCameraAngles = angles; }
...
private:
	...
	glm::vec3 _defaultCameraPosition, _defaultCameraAngles;
