...
bool NewMapDialog::Draw()
{
    ...
    if (ImGui::BeginPopupModal("NEW MAP", &open, ImGuiWindowFlags_AlwaysAutoResize))
    {
        ...
        {
            GetApp()->NewMap(_mapDims[0], _mapDims[1], _mapDims[2]);
            ImGui::EndPopup();
            return false;
        }
...
bool ExpandMapDialog::Draw()
{
    ...
    if (ImGui::BeginPopupModal("EXPAND GRID", &open, ImGuiWindowFlags_AlwaysAutoResize))
    {
        ...
        {
            GetApp()->ExpandMap(_direction, _amount);
            ImGui::EndPopup();
            return false;
        }
...
FileDialog::FileDialog(std::string title, std::initializer_list<std::string> extensions, std::function<void(std::filesystem::path)> callback, bool writeMode)
    ...
    _callback(callback),
    _currentDir(std::filesystem::current_path()),
...
bool FileDialog::Draw()
{
    ...
    if (ImGui::BeginPopupModal(_title.c_str(), &open, ImGuiWindowFlags_AlwaysAutoResize))
    {
        ...
        if (ImGui::BeginListBox("Files", ImVec2(504.0f, 350.0f)))
        {
            ...
            try
            {
                fileIter = std::filesystem::directory_iterator{ _currentDir };
            }
            catch (...)
            {
        ...
        {
            std::filesystem::path newPath = _currentDir / _fileNameBuffer;
            if (_writeMode && std::filesystem::exists(newPath))
            {
...
AssetPathDialog::AssetPathDialog(Settings& settings)
    ...
    _fileDialog(nullptr)
{
    strcpy(_texDirBuffer, GetApp()->GetTexturesDir().c_str());
    strcpy(_shapeDirBuffer, GetApp()->GetShapesDir().c_str());
    strcpy(_defaultTexBuffer, GetApp()->GetDefaultTexturePath().c_str());
    strcpy(_defaultShapeBuffer, GetApp()->GetDefaultShapePath().c_str());
}
...
bool AssetPathDialog::Draw()
{
    ...
    if (ImGui::BeginPopupModal("CONFIGURE ASSET PATHS", &open, ImGuiWindowFlags_AlwaysAutoResize))
    {
        ...
        ImGui::SameLine();
        ImGui::InputText("Default RLTexture", _defaultTexBuffer, TEXT_FIELD_MAX);
        ...
        if (ImGui::Button("Confirm"))
        {
            ...
            bool bad = false;
            std::filesystem::directory_entry texEntry{ _texDirBuffer };
            if (!texEntry.exists() || !texEntry.is_directory())
            {
                strcpy(_texDirBuffer, "Invalid!");
                bad = true;
            }
            std::filesystem::directory_entry defTexEntry{ _defaultTexBuffer };
            if (!defTexEntry.exists() || !defTexEntry.is_regular_file())
            {
                strcpy(_defaultTexBuffer, "Invalid!");
                bad = true;
            }
            std::filesystem::directory_entry shapeEntry{ _shapeDirBuffer };
            if (!shapeEntry.exists() || !shapeEntry.is_directory())
            {
                strcpy(_shapeDirBuffer, "Invalid!");
                bad = true;
            }
            std::filesystem::directory_entry defShapeEntry{ _defaultShapeBuffer };
...
bool ExportDialog::Draw()
{
    ...
    if (ImGui::BeginPopupModal("EXPORT .GLTF/.GLB SCENE", &open, ImGuiWindowFlags_AlwaysAutoResize))
    {
        ...
        {
            _dialog.reset(new FileDialog(std::string("Save .GLTF or .GLB file"), { std::string(".gltf"), std::string(".glb") }, [&](std::filesystem::path path) {
                _settings.exportFilePath = std::filesystem::relative(path).string();
                strcpy(_filePathBuffer, _settings.exportFilePath.c_str());
                }, true));
        }
        ...
        {
            GetApp()->TryExportMap(std::filesystem::path(_settings.exportFilePath), _settings.exportSeparateGeometry);
            GetApp()->SaveSettings();
            ImGui::EndPopup();
            return false;
        }
