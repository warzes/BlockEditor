...
void EntMode::OnExit()
{
    ...
    {
        _ent.model = Assets::GetModel(std::filesystem::path(std::string(_modelPathBuffer)));
    }
    else
    {
        _ent.model = nullptr;
    ...
    {
        _ent.texture = Assets::GetTexture(std::filesystem::path(std::string(_texturePathBuffer)));
    }
    else
    {
...
void EntMode::Update(float deltatime)
{
...
void EntMode::Draw()
{
    ...
    if (ImGui::Begin("##Entity Mode View", &open, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoMove))
    {
        ...
        {
            ImGui::TextUnformatted("RLModel path");
            if (ImGui::Button("Browse##RLModel"))
            {
                _fileDialog.reset(new FileDialog("Select model", { std::string(".obj") },
                    [&](std::filesystem::path path)
                    {
                        std::filesystem::path relativePath = std::filesystem::relative(path);
                        strcpy(_modelPathBuffer, relativePath.string().c_str());
                    },
                    false));
            }
            ImGui::SameLine();
            ImGui::InputText("##RLModel path", _modelPathBuffer, TEXT_FIELD_MAX);

            // Intentional fallthrough
        }
        case Ent::DisplayMode::SPRITE:
        {
            ImGui::TextUnformatted("RLTexture path");
            ...
            ImGui::SameLine();
            ImGui::InputText("##RLTexture path", _texturePathBuffer, TEXT_FIELD_MAX);
